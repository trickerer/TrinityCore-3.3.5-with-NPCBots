...
public:
    ...
    static bool HandleNPCBotRename(ChatHandler* handler, char const* args)
    {
        ...
        WorldDatabase.PExecute("UPDATE `creature_template` SET `name` = '{}' WHERE entry = {}", safeName.c_str(), bot->GetGUID().GetCounter());
    ...
    static bool HandleGetDiscordCodeCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        handler->PSendSysMessage("Join our |cFFFF4747D|cFFFFE547i|cFF7CFF47s|cFF47FFB0c|cFF47B0FFo|cFF7C47FFr|cFFFF47E5d|r Server (|cff00ff00 https://mgawow.online/discord |r)and DM '|cFFFF4747MGAWoW Verify Bot|r' with this code: |cff00ff00%s|r", code.c_str());
        return true;
    }
