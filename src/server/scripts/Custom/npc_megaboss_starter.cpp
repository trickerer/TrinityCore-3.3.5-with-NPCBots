    ...
    struct npc_megaboss_starterAI : public ScriptedAI
    {
        ...
        bool OnGossipSelect(Player* player, uint32 /*menuId*/, uint32 gossipListId) override
        {
            ...
            if (action == GOSSIP_ACTION_INFO_DEF + 1000)
            {
                ...
                me->SummonCreature(NPC_BOSS_5MAN, -9719.334961, 62.957691, -20.552540, 2.947528, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, milliseconds(1800000));
            ...
            if (action == GOSSIP_ACTION_INFO_DEF + 1001)
            {
                CloseGossipMenuFor(player);
                WorldDatabase.PExecute(_QUERY1_);
                me->SummonCreature(NPC_BOSS_10MAN, -9719.334961, 62.957691, -20.552540, 2.947528, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, milliseconds(1800000));
            ...
            if (action == GOSSIP_ACTION_INFO_DEF + 1002)
            {
                CloseGossipMenuFor(player);
                WorldDatabase.PExecute(_QUERY1_);
                me->SummonCreature(NPC_BOSS_25MAN, -9719.334961, 62.957691, -20.552540, 2.947528, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, milliseconds(1800000));
                ChannelMgr* cMgr = ChannelMgr::forTeam(TEAM_NEUTRAL); 
                if (ChannelMgr* channelMgr = ChannelMgr::forTeam(TEAM_NEUTRAL))
                {
                    if (Channel* channel = channelMgr->GetChannel(0, "world", player, false, nullptr))
