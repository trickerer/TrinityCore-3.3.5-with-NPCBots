        ...
        bool OnGossipSelect(Player* player, uint32 /*menuId*/, uint32 gossipListId) override
        {
            ...
            if (action == GOSSIP_ACTION_INFO_DEF + 1000)
            {
                ...
                if (ChannelMgr* channelMgr = ChannelMgr::forTeam(TEAM_NEUTRAL))
                {
                    ...
                    {
                        channel->SayAsFake(3181, "MGA Mega Boss event has started!  5 Man Mode! Prepare yourselves!", LANG_UNIVERSAL);
                    }
                }
                return true;
            }
            if (action == GOSSIP_ACTION_INFO_DEF + 1001)
            {
                ...
                if (ChannelMgr* channelMgr = ChannelMgr::forTeam(TEAM_NEUTRAL))
                {
                    if (Channel* channel = channelMgr->GetChannel(0, "world", player, false, nullptr))
                    {
                        channel->SayAsFake(3181, "MGA Mega Boss event has started! 10 Man Mode! Prepare yourselves!", LANG_UNIVERSAL);
                    }
                }
                return true;
            }
            if (action == GOSSIP_ACTION_INFO_DEF + 1002)
            {
                ...
                if (ChannelMgr* channelMgr = ChannelMgr::forTeam(TEAM_NEUTRAL))
                {
                    if (Channel* channel = channelMgr->GetChannel(0, "world", player, false, nullptr))
                    {
                        channel->SayAsFake(3181, "MGA Mega Boss event has started! 25 Man Mode! Prepare yourselves!", LANG_UNIVERSAL);
                    }
                }
                return true;
            }
