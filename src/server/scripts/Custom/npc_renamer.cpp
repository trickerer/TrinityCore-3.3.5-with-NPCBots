...
public:
    ...
    struct npc_renamerAI : public ScriptedAI
    {
        ...
        {
            WorldDatabase.PExecute("UPDATE `char_rename` SET `status`='{}' WHERE `charid`='{}' AND `status` = '0'", status, pplayer->GetSession()->GetGUIDLow());
            return true;
        }
        ...
        bool OnGossipHello(Player* player) override
        {
			...
			QueryResult result;
            result = WorldDatabase.PQuery("SELECT * FROM `char_rename` WHERE `charid`='{}' AND `status`='0' LIMIT 1", pplayer->GetSession()->GetGUIDLow());
            if (result)
            {
